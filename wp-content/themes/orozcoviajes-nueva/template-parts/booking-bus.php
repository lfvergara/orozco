...
$temp_r = $wpdb->get_results("SELECT $table_reserves.value as val, $table_reserves.status as status, $table_reserves.id as id FROM $table_reserves WHERE $table_reserves.product_id = ".get_the_id()." ;", OBJECT );
$reserves = array();
$reserves_id = array();
...
?>
<!-- Modal -->
<form id="booking_form" action="" autocomplete="off" method="post"  data-toggle="validator">
      ...
      <div class="modal-content">
        ...
        <div id="content-book" class="modal-body">
              <div class="form-group">
                <input type="text" class="form-control" id="booking_name" name="name" value="" required placeholder="Nombre y Apellido" autocomplete="off">
              </div>
              <div class="form-group">
                <input type="text" class="form-control" id="booking_dni" name="dni" value="" required placeholder="DNI" autocomplete="off">
              </div>
              <div class="form-group">
                <input type="date" class="form-control" id="booking_brithday" name="brithday" value="" required placeholder="Fecha de Nacimiento">
              </div>
              <div class="form-group">
...
<div class="modal fade" id="booking_modal_dialog" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Sí</button>
      ...
      <tr>
        <?php for ($i=1;$i <= $filas ;$i++): $row_ident = $planta.'_'.$row.'_'.$i; $value = $bus_meta['booking_plants_'.$row_ident][0]; ?>
          <td class="plants_row_<?= $row.'_'.$i ?>">
            ...
            <button style="background: url(<?php echo plugins_url( 'booking-bus/assets/imgs/seat.png') ?>) no-repeat; background-size: 101%"
              id="<?= $planta.'_'.$row.'_'.$i ?>"
              <?php if ($status_reserve != 'avaliable'): ?>
              ident-reserve="<?= $reserves_id[$row_ident] ?>"
            <?php endif; ?>
              type="button"
              status="<?= $htmlHelper->status_asiento($value,$status_reserve) ?>"
              class="<?= $htmlHelper->status_asiento($value,$status_reserve) ?>">
              <?= $value ?>
            </button>
          </td>
        <?php endfor; ?>
...
<script type="text/javascript">
  window.onload = function (){
    ...
    jQuery('button.avaliable').click(function(){
      ...
      jQuery('#booking_locale').val('');
      jQuery('#asiento_id').val('');
      jQuery('#booking_form').find('input, button').each(function(index, el) {
    ...
    jQuery('button.reserved, button.buyed').click(function(){
      jQuery('#booking_form').find('input, button').each(function(index, el) {
        jQuery(this).prop('disabled',true);
      });
      jQuery.ajax({
      ...
      .done(function(response) {
        jQuery('#booking_name').val(response.name);
        jQuery('#booking_dni').val(response.dni);
        jQuery('#booking_birthday').val(response.brithday);
        jQuery('#booking_phone').val(response.phone);
        jQuery('#booking_domain').val(response.domain);
        jQuery('#booking_postalcode').val(response.postalcode);
        jQuery('#booking_locale').val(response.locale);
        jQuery('#booking_modal').modal('show');
      });
      // jQuery('#asiento_id').val(jQuery(this).attr('id'));
    });
